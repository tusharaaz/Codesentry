...
function LanguageSelector({ selectedOption, setSelectedOption, options, darkMode }) {
  const styles = {
    control: (base, state) => ({ ...base, backgroundColor: darkMode ? "#1e1e1e" : "#ffffff", borderColor: state.isFocused ? "#9333ea" : darkMode ? "#3f3f46" : "#d4d4d8", boxShadow: state.isFocused ? "0 0 0 1px #9333ea" : "none", color: darkMode ? "#ffffff" : "#18181b", width: "200px", cursor: "pointer", "&:hover": { borderColor: "#9333ea" } }),
  ...
  return <Select value={selectedOption} onChange={setSelectedOption} options={options} styles={styles} />;
}
